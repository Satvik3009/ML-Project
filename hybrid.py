...
def _ckpt(name):
    """Return checkpoint path and whether it already exists."""
    p = f"checkpoints/{name}_best.pt"
...
def main():
    ...
    # ── STEP 1: FER2013 ──────────────────────────────────────────────────────
    _banner(f"STEP 1/6 — FER2013 face emotion CNN  (max {mpc} imgs/class)")
    ...
    ckpt_path, already_trained = _ckpt("fer_cnn")
    if already_trained:
        print(f"  [fer_cnn] Checkpoint found — loading {ckpt_path}, skipping training.")
        fer_model.load_state_dict(torch.load(ckpt_path, map_location=DEVICE))
    else:
    ...
    # ── STEP 2: fMRI ─────────────────────────────────────────────────────────
    _banner(f"STEP 2/6 — fMRI 3D CNN  (max {CFG['MAX_FMRI_SUBJECTS']} subjects)")
    ...
    ckpt_path, already_trained = _ckpt("fmri_cnn")
    if already_trained:
        print(f"  [fmri_cnn] Checkpoint found — loading {ckpt_path}, skipping training.")
        fmri_model.load_state_dict(torch.load(ckpt_path, map_location=DEVICE))
    else:
    ...
    # ── STEP 3: EEG ──────────────────────────────────────────────────────────
    _banner(f"STEP 3/6 — EEG MLP  (max {CFG['MAX_DEAP_SUBJECTS']} DEAP subjects)")
    ...
    ckpt_path, already_trained = _ckpt("eeg_mlp")
    if already_trained:
        print(f"  [eeg_mlp] Checkpoint found — loading {ckpt_path}, skipping training.")
        eeg_model.load_state_dict(torch.load(ckpt_path, map_location=DEVICE))
    else:
    ...
    ckpt_path, already_trained = _ckpt("fusion")
    if already_trained:
        print(f"  [fusion] Checkpoint found — loading {ckpt_path}, skipping training.")
        fusion_model.load_state_dict(torch.load(ckpt_path, map_location=DEVICE))
        best_fusion_acc = 0.0
    else:
        print("  Training fusion layer on FER training data...")
        fusion_opt  = torch.optim.Adam(fusion_model.parameters(), lr=CFG["lr"])
        fusion_crit = nn.CrossEntropyLoss()
        best_fusion_acc = 0.0
        n_epochs_fusion = CFG["epochs_emotion"]

        for ep in range(1, n_epochs_fusion + 1):
            fusion_model.train()
            total_loss, correct, n = 0.0, 0, 0
            bar = tqdm(fer_tr, desc=f"  [fusion] ep {ep}/{n_epochs_fusion} train",
                       total=len(fer_tr), ncols=80, leave=False)
            for imgs, labels in bar:
                ...
                _, fused_logits = fusion_model(fer_logits, fmri_dummy, eeg_dummy)
            ...
            with torch.no_grad():
                for v_imgs, v_labels in fer_val:
                    ...
                    fd_f   = torch.zeros(B, n_fmri, device=DEVICE)
                    fd_e   = torch.zeros(B, n_eeg,  device=DEVICE)
                    _, v_log = fusion_model(fer_lg, fd_f, fd_e)
            ...
            if val_acc > best_fusion_acc:
                ...
                flag = "  <- NEW BEST"
            print(f"  [fusion] ep {ep:2d}/{n_epochs_fusion} | "
    ...
    ckpt_path, already_trained = _ckpt("tumor_cnn")
    if already_trained:
        print(f"  [tumor_cnn] Checkpoint found — loading {ckpt_path}, skipping training.")
    else:
