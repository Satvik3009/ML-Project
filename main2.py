...
try:
    ...
    # Fallback tqdm that just prints lines if tqdm is not installed
    class tqdm:
        ...
        def __iter__(self):
            for item in self.iterable:
                ...
                self._n += 1
                if self._n % 10 == 0:
                    print(f"  {self.desc}: {self._n}/{self.total or '?'}", flush=True)
...
def main():
    ...
    # ── STEP 4: Generate emotion embeddings for tumor .mat samples ──────────────
    _banner("STEP 4/5 — Generate emotion vectors for tumor MRI images")
    fer_model.load_state_dict(torch.load("checkpoints/fer_cnn_best.pt", map_location=DEVICE))
    fer_model.eval()

    # Load the same .mat files used for tumor training (with same cap)
    probe_tf = T.Compose([
        T.Resize((48, 48)), T.ToTensor(),
        T.Normalize([0.485,0.456,0.406],[0.229,0.224,0.225])
    ])
    # Re-use load_tumor_mat_files to get the exact same file list
    ...
    with torch.no_grad():
        for b_i, (imgs, _, _) in enumerate(
                tqdm(probe_loader, desc="  Generating emotion vecs", ncols=80)):
    ...
    print(f"  Emotion vectors ready for {len(emotion_vectors):,} MRI samples.")

    # ── STEP 5: Tumor classifier ──────────────────────────────────────────────
    _banner(f"STEP 5/5 — Brain Tumor CNN  (max {mpc} imgs/class)")
